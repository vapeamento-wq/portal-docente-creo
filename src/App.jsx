...
const App = () => {
          ...
          for (let i = 55; i <= 61; i++) { 
            ...
            if (texto && texto !== "-" && texto !== "") {
              ...
              semanas.push({
                ...
                hora: texto.match(/(\d{1,2}\s*A\s*\d{1,2})/i)?.[0] || (partes[1] ? partes[1].trim() : "Consultar"),
  ...
  if (state.loading) return <div style={{textAlign:'center', padding:'100px'}}>Cargando v2.0...</div>;

  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: 'sans-serif', position: 'relative' }}>
      {/* MARCA DE VERIFICACIÓN */}
      <div style={{ position: 'fixed', bottom: 10, right: 10, fontSize: '10px', color: '#ccc', zIndex: 1000 }}>v2.0-Sincronizado</div>
      ...
      <main className="container">
        {!docente ? (
          <div style={{gridColumn:'1/-1', textAlign:'center', padding:'100px'}}>
            <h2>Bienvenido</h2>
            <p>Ingrese su identificación para consultar (v2.0).</p>
          </div>
        ) : (
          <>
              ...
              <div className="weeks">
                {cursoActivo.semanas.map((s, idx) => (
                  <div key={idx} className="week-card">
                    ...
                    {s.zoomId && (
                      <a href={s.zoomLink} target="_blank" rel="noreferrer" className="zoom-link">ENTRAR A ZOOM</a>
                    )}
                  </div>
                ))}
              </div>
