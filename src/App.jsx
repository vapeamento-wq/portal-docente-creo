...
const App = () => {
    ...
    fetch(URL_CSV)
      ...
      .then(csvText => {
        ...
        filas.forEach((fila) => {
          ...
          // 1. FILTRO: Si la materia dice "PENDIENTE", ignoramos la fila completa
          ...
          const fFin = c[13]?.replace(/"/g, '').trim();    

          const semanas = [];
          for (let i = 14; i <= 29; i++) { 
            const texto = c[i]?.replace(/"/g, '').trim() || "";
            
            // 2. FILTRO: Solo mostrar semanas con información real (no 0, no -, no pendiente)
          ...
          if (id && !isNaN(id)) {
            ...
            // Solo agregar el curso si tiene al menos una semana con horario
            if (semanas.length > 0) {
  ...
  const handleSearch = (e) => {
    ...
    } else { alert("Cédula no encontrada o sin cursos programados."); }
  };

  if (state.loading) return <div style={{textAlign:'center', padding:'100px', color:'#004A87', fontWeight:'bold'}}>Aplicando filtros de seguridad v2.3...</div>;
      ...
      <header style={{ background: '#004A87', color: 'white', padding: '15px 20px', borderBottom: '4px solid #D4AF37' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <form onSubmit={handleSearch} style={{ display: 'flex', background: 'white', borderRadius: '8px', padding: '2px' }}>
            <input type="text" placeholder="Identificación..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ border:'none', padding:'8px', outline:'none', width:'160px'}} />
      ...
      <main style={{ maxWidth: '1200px', margin: '30px auto', padding: '0 20px', display: 'grid', gridTemplateColumns: docente ? '300px 1fr' : '1fr', gap: '25px' }}>
        {!docente ? (
          <div style={{textAlign:'center', padding:'80px', color:'#94a3b8'}}>
            ...
            <p>Se han filtrado los resultados para mostrar únicamente cursos confirmados.</p>
          </div>
        ) : (
          <>
            <aside style={{ background: 'white', borderRadius: '15px', padding: '20px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', height: 'fit-content' }}>
              ...
              <p style={{fontSize:'0.65rem', color:'#D4AF37', fontWeight:'bold', marginBottom:'20px'}}>DOCENTE ACTIVO</p>
            ...
            <section style={{ background: 'white', borderRadius: '15px', overflow: 'hidden', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}>
              <div style={{ background: '#004A87', color: 'white', padding: '20px' }}>
                ...
                <p style={{margin:'5px 0 0', fontSize:'0.7rem', color:'#D4AF37'}}>CRÉDITOS: {cursoActivo.creditos} | PROGRAMACIÓN DE 16 SEMANAS</p>
              </div>
