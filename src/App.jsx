...
const App = () => {
  ...
  // --- MEMORIA LOCAL (NO FALLA) ---
  const [logs, setLogs] = useState(() => {
    try {
      const saved = localStorage.getItem('portal_logs_emergency');
  ...
  const handleSearch = (e) => {
    ...
    setLogs(nuevosLogs);
    localStorage.setItem('portal_logs_emergency', JSON.stringify(nuevosLogs));
  ...
  const clearLogs = () => {
    if(window.confirm("¿Limpiar historial local?")) {
      setLogs([]);
      localStorage.removeItem('portal_logs_emergency');
    }
  };
      ...
      <div style={{fontFamily:'Segoe UI, sans-serif', background:'#f4f6f8', minHeight:'100vh', padding:'20px'}}>
        <div style={{maxWidth:'1000px', margin:'0 auto'}}>
          <header style={{background:'#2c3e50', color:'white', padding:'20px', borderRadius:'10px', display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'30px'}}>
            <div>
              <h2 style={{margin:0}}>PANEL DE EMERGENCIA</h2>
